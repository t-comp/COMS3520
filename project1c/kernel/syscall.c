...
/*
 * system call handler, gets pid from user space and calls 
 * blockchild() to stop the child from being scheduled
 */
uint64 sys_blockchild(void) {
  int pid;
  argint(0, &pid);
...
/*
 * system call handler, gets pid from user space and calls 
 * unblockchild() to allow the child to be scheduled again
 */
uint64 sys_unblockchild(void) {
  int pid;
  argint(0, &pid);
...
/*
 * copies resource usage counters of the calling process to 
 * user space and retyrns the callers pid
 */
uint64
sys_getresourceusage(void)
{
  ...
  // populate usage struct from process counters
